          ...
          <div class="p-3 right-side">
            ...
            <div v-if="product?.productDetailList?.length > 0" class="text-start">
              ...
              <div class="d-flex flex-row ">
                <button :class="{'bg-success text-light' : size == item.size.size}" type="button" class="m-1"
                        v-for="item in product?.productDetailList" @click.prevent="size = item.size.size;getQuantity()">
                  {{ item?.size?.size }}
                </button>
              </div>
            </div>
