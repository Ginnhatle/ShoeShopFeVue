			...
			<div class="row g-0">
				...
				<div class="col-md-6">
					<div class="p-3 right-side">
						...
						<div class="text-start">
							Giảm giá: <span
							class="fw-bold text-danger discount">{{ product.discount + '%' }}</span>
						</div>
						<!--						thuôc tính-->
						<div class="text-start">
						...
						<div class="buttons d-flex flex-row mt-5 gap-3">
							<button type="button" class="btn btn-dark" @click.prevent="addToCart()">Thêm vào giỏ hàng
							</button>
						</div>
	...
	methods: {
		getProduct() {
			this.productService.findById(Number.parseInt(this.productId)).then((response) => {
				...
				if (this.product && this.product.imageThumbnail) {
					this.productImageThumbnail = this.product.imageThumbnail;
				}
			}).catch((error) => {
				console.log(error);
			});
		...
		changeImage(productImageThumbnail: string) {
...
.small_images img {
	width: 80px; /* Độ rộng của mỗi ảnh nhỏ */
